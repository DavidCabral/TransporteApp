...
begin
  ...
  //atualizando tabela de socio, adicionando o campo sig_instituicao se nao existir
  qryGeral.Close;
  qryGeral.SQL.Text:=' SELECT R.RDB$FIELD_NAME                    '#13+
                     ' FROM RDB$RELATION_FIELDS R                 '#13+
                     ' WHERE R.RDB$RELATION_NAME = ''INSTITUICAO'' AND  '#13+
                     '       R.RDB$FIELD_NAME = ''SIG_INSTITUICAO'' ';
  qryGeral.Open;

  if qryGeral.IsEmpty then
  begin
    qryGeral.Close;
    qryGeral.SQL.Text:='ALTER TABLE INSTITUICAO ADD SIG_INSTITUICAO VARCHAR(20)';
    qryGeral.ExecSQL;
    FBTrans.CommitRetaining;
  end;
