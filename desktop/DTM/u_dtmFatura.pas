...
type
  ...
  public
    ...
    function salvarFatura(pFatura: TFatura; pControle: TControle;
      pCommit: boolean = False): boolean;
    function getFatura(pCpf: string; pMes, pAno: integer): TFatura;
    function deleteFatura(pCpf: string; pMes, pAno: integer;
      pCommit: boolean = False): boolean;
...
begin
  ...
  try
    ...
    begin
      ...
      qryFatura.SQL.Text :=
        ' INSERT INTO FATURA (CPF, COD_COBRANCA, DATA_GERACAO, DATA_PAGAMENTO, VALOR, MES, ANO, VENCIMENTO) '#13 +
...
begin
  try
    qryFatura.Close;
    qryFatura.SQL.Text :=
      'SELECT CPF FROM Fatura WHERE CPF = :CPF AND MES = :MES AND ANO = :ANO AND DATA_PAGAMENTO IS NOT NULL ';
    qryFatura.ParamByName('CPF').AsString := pCpf;
    qryFatura.ParamByName('MES').AsInteger := pMes;
    qryFatura.ParamByName('ANO').AsInteger := pAno;
    qryFatura.Open;
    ...
    qryFatura.Close;
    qryFatura.SQL.Text :=
      'DELETE FROM Fatura WHERE CPF = :CPF AND MES = :MES AND ANO = :ANO ';
    qryFatura.ParamByName('CPF').AsString := pCpf;
    qryFatura.ParamByName('MES').AsInteger := pMes;
    qryFatura.ParamByName('ANO').AsInteger := pAno;
    qryFatura.ExecSQL;
